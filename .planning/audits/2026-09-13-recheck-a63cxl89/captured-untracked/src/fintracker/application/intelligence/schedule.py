...
def _weekly_due(preference: AnalysisPreference, local_now: dt.datetime) -> bool:
    if not preference.weekly_enabled:
        return False
    if local_now.weekday() != preference.weekly_weekday:
        return False
    return local_now.hour >= preference.weekly_hour
...
async def due_analyses(
    ...
    *,
    ...
    if preference.plan_preparation_enabled and 0 <= (period.end_exclusive - today).days <= (
        PLAN_LEAD_DAYS
    ):
        due.append(
            ScheduledAnalysis(
    ...
    if preference.closing_enabled and 0 <= (today - period.start_date).days < CLOSING_GRACE_DAYS:
...
async def handle_run_analysis(settings: Settings, job: LeasedJob) -> None:
    ...
    AI вызывается вне команды записи денег; результат сохраняется карточками,
    а доставка участникам идёт обычным путём outbox → персональные доставки,
    где применяются тихие часы и личные отключения.
    """
