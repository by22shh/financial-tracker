...
from fintracker.db.models.platform import InboundEvent, Job, Draft
...
from fintracker.application.conversation.service import record_free_text, handle
...
async def prepared(owner_session):
    ...
    await owner_session.execute(update(Membership).where(Membership.id==f.actor.membership_id).values(autopost_enabled=True))
    await owner_session.commit()
    return f

def incoming(f, text, *, chat_id=None, update_id=101, message_id=11, edited=False):
    body = {"message_id":message_id, "date":int(dt.datetime.now(dt.UTC).timestamp()),
        "from":{"id":f.user.telegram_user_id,"is_bot":False,"first_name":"Audit"},
        "chat":{"id":chat_id or f.user.telegram_user_id, "type":"supergroup" if chat_id and chat_id<0 else "private"},
        "text":text}
...
async def tx_count(settings, f):
    async with session_scope(settings,RuntimeRole.OWNER) as s:
        return await s.scalar(select(func.count()).select_from(Transaction).where(Transaction.workspace_id==f.workspace.id))
...
async def test_audit_ai_enabled_text_can_finish(owner_session, test_settings):
    ...
    provider=ScriptedAIProvider(responses=[extraction_json(candidate={"category_id":str(f.categories["Продукты"]),"date_expression":None})])
    set_provider_override(provider)
    try:
        result=await record_free_text(configured,actor=f.actor,workspace=f.workspace,
...
async def test_audit_restart_after_post_does_not_duplicate(owner_session,test_settings):
    ...
    try:
        ...
        await handle_process_inbound_event(test_settings,job)
        assert await tx_count(test_settings,f)==1, "Same inbound event created two financial transactions"
    finally:
...
async def test_audit_stale_lease_cannot_post(owner_session,test_settings):
    ...
    try:
        await handle_process_inbound_event(test_settings,job)
        assert await tx_count(test_settings,f)==0, "Worker posted money before checking its lost lease"
    finally:
        set_sender_override(None)
...
async def test_audit_failed_immediate_reply_remains_retryable(owner_session,test_settings):
    ...
    try:
        ...
        assert state!="processed", "Failed reply was acknowledged as processed and will never be retried"
    finally:
        set_sender_override(None)
...
async def test_audit_group_message_does_not_disclose_private_budgets(owner_session,test_settings):
    ...
    try:
        await handle_process_inbound_event(test_settings,job)
        exposed=[m for m in sender.sent if m["chat_id"]==group and f.workspace.name in m["text"]]
        assert not exposed, "Private budget name was sent to all members of an unrelated Telegram group"
    finally:
        set_sender_override(None)

async def test_audit_multiple_edits_are_accepted(owner_session,test_settings):
    f=await prepared(owner_session)
    await accept_telegram_update(test_settings,incoming(f,"продукты 500",update_id=201,message_id=99))
    await accept_telegram_update(test_settings,incoming(f,"продукты 600",update_id=202,message_id=99,edited=True))
    accepted=await accept_telegram_update(test_settings,incoming(f,"продукты 700",update_id=203,message_id=99,edited=True))
    assert not accepted.duplicate

async def test_audit_transfer_is_not_converted_into_expense(owner_session,test_settings):
    from fintracker.application.conversation.entry import create_draft_with_candidates, ExtractionResult, CandidateFields
    ...
    f=await prepared(owner_session)
    async with session_scope(test_settings,RuntimeRole.API,user_id=f.user.id,workspace_id=f.workspace.id) as s:
        draft,_=await create_draft_with_candidates(s,settings=test_settings,actor=f.actor,source_kind="text",
            raw_text="перевёл 500 с карты на кошелёк",extraction=ExtractionResult(intent=Intent.RECORD_TRANSACTION,
                candidates=[CandidateFields(kind="transfer",amount_minor=50000,currency="RUB",occurred_date=dt.date.today(),
                    category_id=f.categories["Продукты"])]))
        draft_id=draft.id
    await confirm_draft(test_settings,actor=f.actor,workspace=f.workspace,draft_id=draft_id,origin="telegram_text")
    async with session_scope(test_settings,RuntimeRole.OWNER) as s:
        kinds=(await s.scalars(select(TransactionRevision.transaction_type).where(TransactionRevision.workspace_id==f.workspace.id))).all()
...
async def test_audit_export_import_buttons_are_wired(owner_session,test_settings,button):
    f=await prepared(owner_session)
    replies=await handle(test_settings,IncomingMessage(telegram_user_id=f.user.telegram_user_id,
        chat_id=f.user.telegram_user_id,kind=MessageKind.CALLBACK,callback_data=button,workspace_id=f.workspace.id))
    assert not any("Кнопка устарела" in r.text for r in replies), f"{button} displayed by menu but has no handler"

async def test_audit_expired_lease_is_invalid_without_reclaim(owner_session,test_settings):
    f=await prepared(owner_session)
    job=await leased(test_settings,incoming(f,"/start"))
    async with session_scope(test_settings,RuntimeRole.OWNER) as s:
        await s.execute(update(Job).where(Job.id==job.id).values(lease_until=dt.datetime.now(dt.UTC)-dt.timedelta(seconds=5)))
