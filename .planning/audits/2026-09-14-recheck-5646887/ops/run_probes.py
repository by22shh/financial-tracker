...
shutil.copy2(OUT/'test_ops_recheck.py',snapshot/'tests/integration/test_ops_recheck.py')
...
with (OUT/'pytest.log').open('w') as log:r=subprocess.run([str(snapshot/'.venv/bin/python'),'-m','pytest','-q','--tb=short','tests/integration/test_ops_recheck.py','--junitxml='+str(OUT/'junit.xml')],cwd=snapshot,env=env,stdout=log,stderr=subprocess.STDOUT)
