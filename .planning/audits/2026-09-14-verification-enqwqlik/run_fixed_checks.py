...
def main() -> int:
    ...
    snapshot = Path(tempfile.mkdtemp(prefix="fintracker-verification4-"))
    head, hashes = capture(repo, snapshot)
    (snapshot/".venv").symlink_to(repo/".venv", target_is_directory=True)
    shutil.copy2(evidence/"test_verification4_cases.py", snapshot/"tests/integration/test_verification4_cases.py")
    ...
    with (output/"pytest.log").open("w") as log:
        result = subprocess.run([
            ...
            "tests/integration/test_verification4_cases.py", "-q", "--tb=short",
