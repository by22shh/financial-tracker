...
shutil.copy2(OUT/'test_contract_readiness.py',snapshot/'tests/integration/test_contract_readiness.py')
env={k:v for k,v in os.environ.items() if not k.startswith('FINTRACKER_')};env.update(FINTRACKER_TEST_DB='fintracker_ops_ready_'+uuid.uuid4().hex[:10],FINTRACKER_TEST_PG_PORT='55432',FINTRACKER_AI__ENABLED='false',FINTRACKER_ASR__PROVIDER='none',PYTHONPATH=str(snapshot/'src'))
...
with (OUT/'pytest.log').open('w') as log:r=subprocess.run([str(snapshot/'.venv/bin/python'),'-m','pytest','-q','--tb=short','tests/integration/test_contract_readiness.py','--junitxml='+str(OUT/'junit.xml')],cwd=snapshot,env=env,stdout=log,stderr=subprocess.STDOUT)
