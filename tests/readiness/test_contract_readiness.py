...
@pytest.mark.parametrize(
    "model", [ExtractionResponse, ReceiptResponse, RecommendationResponse, AnalyticsPlan]
)
    ...
    def walk(value, path="$"):
        if isinstance(value, dict):
            if isinstance(value.get("type"), list):
                violations.append((path, ["type-array-is-not-supported"]))
...
async def test_image_bytes_validated_before_paid_provider(
    ...
    async def download(settings, *, file_id):
        return b"This is plain text, not a JPEG, PNG or WebP image."
