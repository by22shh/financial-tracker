...
_PATTERNS: tuple[tuple[Intent, re.Pattern[str]], ...] = (
    (
        # Попытка управлять системой текстом не становится записью: указания
        # внутри содержимого — это данные, а не команды (SEC-06, AI-05).
        Intent.UNKNOWN,
        re.compile(
            r"\bигнорируй\b|\bзабудь\b.{0,30}\b(предыдущ|прежн|указани|инструкц)|"
            r"\bпредыдущие\s+(указания|инструкции)\b|"
            r"^\s*(система|system|assistant|разработчик)\s*:",
            re.IGNORECASE,
        ),
    ),
    (
    ...
    (
        ...
        re.compile(
            ...
            r"\bне бери\b|\bне записыв|\bничего не (?:купил|покупал|тратил|потратил)|"
            # «В магазине не было молока, взял кефир» — покупка всё же была:
            # «не было» отменяет только саму покупку или трату.
            r"\bне оплач(?:ивал|ена|ен)\b|"
            r"\b(?:покупки|траты|расхода|оплаты)\s+не\s+было\b|"
            r"\bне\s+было\s+(?:покупки|траты|расхода|оплаты)\b|\bне\s+было\s*[.!]?\s*$|"
            r"^\s*отмена\b(?:\s*[.!]?\s*$|\s*[,:—-])|"
    ...
    (
        ...
        re.compile(
            ...
            # «это Транспорт» без суммы — перенос последней записи в категорию.
            r"^\s*это\s+(?:в\s+)?[^\d\s][^\d]{1,40}$",
            re.IGNORECASE,
        ),
    ),
    (
        ...
        re.compile(
            r"\bудал(?:и|ить)\s+(?:эту\s+|последнюю\s+|прошлую\s+)?(?:трату|запись|покупку)\b|"
            r"\bэто дубл|\bвн[её]с(?:ла)? по ошибке\b|"
            r"\bотмени(?:ть)?\s+(?:эту\s+|последнюю\s+)?(?:запись|трату|покупку)\b",
            re.IGNORECASE,
        ),
    ),
    (
    ...
    (
        Intent.FUTURE_PLAN,
        re.compile(
            # Признак будущего вместе с суммой — план, даже без глагола:
            # «на следующей неделе такси 500» не является совершённой тратой.
            r"\b(завтра|послезавтра|на следующей неделе|на след неделе|"
            r"в следующем месяце)\b.{0,60}?(\b(куплю|оплачу|плачу|заплачу)\b|\d)|"
            # Намерение в будущем времени без даты — тоже план, а не трата.
            r"\b(надо|нужно|планиру\w*|собира\w*с[ья])\b.{0,40}?"
            r"\b(будет\s+)?(заплат|оплат|купить|купл|потрат)",
            re.IGNORECASE,
        ),
    ),
)
...
# Перевод — только между своими счетами. «Перевёл Маше 5000» — это оплата
# другому человеку, то есть расход, а не перемещение денег.
_TRANSFER_WORDS = re.compile(
    r"(?<![а-яё])("
    r"перев[её]л[аи]?\s+(?:\d[\d\s.,]*\s*(?:р|руб\w*|₽)?\s+)?"
    r"(?:себе|между|на\s+(?:свою|свой|мою|мой|вклад|депозит|накопит\w*|сберегат\w*|"
    r"брокер\w*|сбережени\w*)|с[о]?\s+(?:своей|своего|моей|моего|карты|счёта|счета|вклада))|"
    r"перевод\s+(?:между|себе|на\s+(?:вклад|накопит\w*|свою|свой))|"
    r"снял[аи]?\s+наличн|снятие\s+наличн|"
    r"положил[аи]?\s+на\s+(?:вклад|депозит|сч[её]т|накопит\w*|карту)"
    r")",
    re.IGNORECASE,
)
