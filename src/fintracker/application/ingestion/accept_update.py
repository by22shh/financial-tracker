"""Долговечный приём обновления Telegram (TECH-01, TECH-02, CMD-01, NFR-01).

До ответа 2xx обновление и первая задача сохраняются одной транзакцией.
Уровни совпадения (TECH-07): точный технический повтор — по (bot_id, update_id);
логическое сообщение — по (chat_id, message_id, edit_version); хэш файла и
фискальные реквизиты остаются признаками кандидата дубликата.
